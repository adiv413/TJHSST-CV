...
PointPair random_algo(vector<Point>& points) {
    ...
    unordered_map<pair<ll, ll>, Point, HashFunction> point_map;

    for(int i = 0; i < points.size(); i++) {
        Point curr = points[i];
        ll r = (ll) (curr.getX() / (delta / 2) + 0.5);
        ll c = (ll) (curr.getY() / (delta / 2) + 0.5);
        bool broken = false;
        
        for(ll j = r - 2; (j <= r + 2) && !broken; j++) {
            for(ll k = c - 2; k <= c + 2; k++) {
                
                // cout << (int) point_map.count(make_pair(j, k)) << " " << j << " " << k << endl;

                if(point_map.count(make_pair(j, k)) > 0 && point_map[make_pair(j, k)].distance(curr) < delta) {
                    // update delta, update minimum, clear the map, refill map
                    
                    delta = curr.distance(point_map[make_pair(j, k)]);
                    minimum = PointPair(point_map[make_pair(j, k)], curr, delta);
                    point_map.clear();

                    // add all points from 0 to i to map based on new delta
                    for(int l = 0; l <= i; l++) {
                        ll r = (ll) (points[l].getX() / (delta / 2) + 0.5);
                        ll c = (ll) (points[l].getY() / (delta / 2) + 0.5);
                        point_map[make_pair(r, c)] = points[l];
                    }
                    broken = true;
                    break;
                }
