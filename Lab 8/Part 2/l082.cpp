...
void part2() {
    ...
    for(int frame_num = 0; frame_num <= frames; frame_num++) {
        ...
        for(int i = 0; i < cube.size(); i++) {
            Mat m = rotate_z(rotate_y(rotate_x(cube[i], angle_increment * frame_num), angle_increment * frame_num), angle_increment * frame_num);
            std::vector<double> out(m.begin<double>(), m.end<double>());
            
            double eye = 12;
            double plane = 5;
            
            double t = (plane - eye) / (out[0] - eye);
            double u = out[1] * t;
            double v = out[2] * t;
            
            Mat projected_point = (Mat_<double>(4, 1) << u, v, 1, 1);
            Mat final_point = translate(scale(projected_point, 100, 100, 100), 400, 300, 0);
            std::vector<double> final_out(final_point.begin<double>(), final_point.end<double>());
            ...
            if(frame_num < 4) {
                if(i != cube.size() - 1) {
                    results << "(" << final_out[0] << "," << final_out[1] << "," << final_out[2] << "), ";
                }
                else {
                    results << "(" << final_out[0] << "," << final_out[1] << "," << final_out[2] << ")" << endl;
                }
            }
    ...
    for(int frame_num = 0; frame_num <= frames; frame_num++) {
        ...
        for(int i = 0; i < tetrahedron.size(); i++) {
            Mat m = rotate_z(rotate_y(rotate_x(tetrahedron[i], angle_increment * frame_num), angle_increment * frame_num), angle_increment * frame_num);
            std::vector<double> out(m.begin<double>(), m.end<double>());
            ...
            Mat projected_point = (Mat_<double>(4, 1) << u, v, 1, 1);
            Mat final_point = translate(scale(projected_point, 250, 250, 250), 400, 300, 0);
            std::vector<double> final_out(final_point.begin<double>(), final_point.end<double>());
